var V={},H=(N,C,L)=>(V.__chunk_2916=(B,P,f)=>{"use strict";f.d(P,{E:()=>$});class v{constructor(e){this.vehicleRepository=e}async getAvailableVehicles(e){if(e&&!this.isValidVehicleType(e))throw Error(`Invalid vehicle type: ${e}`);let t=await this.vehicleRepository.getAvailableVehicles(e);return this.sortVehiclesByPriority(t)}async getVehicleById(e){if(!e||e.trim()==="")throw Error("Vehicle ID is required");return this.vehicleRepository.getVehicleById(e)}async getVehicleCount(){return this.vehicleRepository.getVehicleCount()}async getVehiclesForAdmin(e){return this.vehicleRepository.getVehiclesByAdmin(e)}async createVehicle(e){this.validateVehicleData(e);let t=await this.vehicleRepository.createVehicle(e),i=await this.vehicleRepository.getVehicleById(t);if(!i)throw Error("Failed to create vehicle");return i}async updateVehicleAvailability(e,t){if(!e||e.trim()==="")throw Error("Vehicle ID is required");await this.vehicleRepository.updateVehicleAvailability(e,t);let i=await this.vehicleRepository.getVehicleById(e);return!!i&&i.isAvailable===t}isValidVehicleType(e){return["sedan","noah","hiace"].includes(e.toLowerCase())}validateVehicleData(e){if(!e.name||e.name.trim()==="")throw Error("Vehicle name is required");if(!e.type||!this.isValidVehicleType(e.type))throw Error("Valid vehicle type is required (sedan, noah, hiace)");if(!e.capacity||e.capacity<1)throw Error("Vehicle capacity must be at least 1");if(!e.pricePerDay||e.pricePerDay<0)throw Error("Price per day must be a positive number")}sortVehiclesByPriority(e){return e.sort((t,i)=>{let r=t.photos.some(a=>a.isPrimary),s=i.photos.some(a=>a.isPrimary);return r&&!s?-1:!r&&s?1:new Date(i.createdAt).getTime()-new Date(t.createdAt).getTime()})}async getAllVehicles(){return this.vehicleRepository.getAllVehicles()}async addVehiclePhoto(e,t){return this.vehicleRepository.addVehiclePhoto(e,t)}async setPrimaryPhoto(e,t){return this.vehicleRepository.setPrimaryPhoto(e,t)}async deleteVehiclePhoto(e){return this.vehicleRepository.deleteVehiclePhoto(e)}async updateVehicle(e,t){this.validateVehicleData(t),await this.vehicleRepository.updateVehicle(e,t);let i=await this.vehicleRepository.getVehicleById(e);if(!i)throw Error("Failed to fetch updated vehicle");return i}async deleteVehicle(e){if(!await this.vehicleRepository.getVehicleById(e))throw Error("Vehicle not found");return await this.vehicleRepository.deleteVehicle(e),!await this.vehicleRepository.getVehicleById(e)}}function l(n){let e=n.replace(/\D/g,"");return e.length===10&&!e.startsWith("880")||e.length===11&&e.startsWith("01")?`+880${e}`:n.startsWith("+")?n:`+${e}`}function E(n){let e=n.replace(/\D/g,"");return e.length>=10&&e.length<=15}class T{constructor(e,t,i){this.bookingRepository=e,this.passengerRepository=t,this.vehicleRepository=i}async createBooking(e){this.validateBookingData(e);let t=null;if(e.vehicleId&&e.vehicleId!=="pending-assignment"){if(!(t=await this.vehicleRepository.getVehicleById(e.vehicleId)))throw Error("Vehicle not found");if(!t.isAvailable)throw Error("Vehicle is not available for booking")}let i=l(e.passengerPhone),r=await this.passengerRepository.findByPhone(i);return r?await this.passengerRepository.updatePassenger(r.id,{name:e.passengerName,email:e.passengerEmail||r.email}):r=await this.passengerRepository.create({phone:i,name:e.passengerName,email:e.passengerEmail,isVerified:!1}),await this.bookingRepository.create({passengerId:r.id,vehicleId:e.vehicleId||"pending-assignment",bookingDate:e.bookingDate,pickupTime:e.pickupTime,tripType:e.tripType,pickupLocation:e.pickupLocation,dropoffLocation:e.dropoffLocation,status:"pending"})}async getBookingById(e){return this.bookingRepository.findById(e)}async getBookings(e={}){return this.bookingRepository.findAll(e)}async getBookingsByPassengerPhone(e){let t=l(e),i=await this.passengerRepository.findByPhone(t);return i?this.bookingRepository.findByPassengerId(i.id):[]}async updateBookingStatus(e,t){if(!await this.bookingRepository.findById(e))throw Error("Booking not found");return this.bookingRepository.updateStatus(e,t)}async updateBooking(e,t){if(!await this.bookingRepository.findById(e))throw Error("Booking not found");return await this.bookingRepository.updateBooking(e,t),this.bookingRepository.findById(e)}async cancelBooking(e,t){let i=await this.bookingRepository.findById(e);if(!i)throw Error("Booking not found");if(i.status==="cancelled")throw Error("Booking is already cancelled");if(i.status==="completed")throw Error("Cannot cancel completed booking");return this.bookingRepository.updateBooking(e,{status:"cancelled",notes:t?`Cancelled: ${t}`:"Cancelled by user"})}async getBookingStats(){let{bookings:e}=await this.bookingRepository.findAll({limit:1e3}),t={total:e.length,pending:0,confirmed:0,completed:0,cancelled:0};return e.forEach(i=>{switch(i.status){case"pending":t.pending++;break;case"confirmed":t.confirmed++;break;case"completed":t.completed++;break;case"cancelled":t.cancelled++}}),t}validateBookingData(e){if(!e.bookingDate)throw Error("Booking date is required");if(!e.pickupTime)throw Error("Pickup time is required");if(!e.tripType)throw Error("Trip type is required");if(!e.pickupLocation)throw Error("Pickup location is required");if(e.tripType==="round"&&!e.dropoffLocation)throw Error("Drop-off location is required for round trip");if(!e.passengerName)throw Error("Passenger name is required");if(!e.passengerPhone)throw Error("Passenger phone is required");if(!E(e.passengerPhone))throw Error("Invalid phone number format");if(e.passengerEmail&&!/\S+@\S+\.\S+/.test(e.passengerEmail))throw Error("Invalid email format");if(!e.vehicleId)throw Error("Vehicle selection is required");let t=new Date(e.bookingDate),i=new Date;if(i.setHours(0,0,0,0),t<i)throw Error("Booking date cannot be in the past")}async checkAvailability(e,t){let i=await this.vehicleRepository.getVehicleById(e);return!!i&&!!i.isAvailable}async getBookingsByVehicleId(e){return this.bookingRepository.findByVehicleId(e)}}class _{constructor(e,t){this.passengerRepository=e,this.bookingRepository=t}async createPassenger(e){if(!E(e.phone))throw Error("Invalid phone number format");let t=l(e.phone);if(await this.passengerRepository.findByPhone(t))throw Error("Passenger with this phone number already exists");if(e.email&&!/\S+@\S+\.\S+/.test(e.email))throw Error("Invalid email format");return this.passengerRepository.create({phone:t,name:e.name,email:e.email,isVerified:!1})}async getPassengerById(e){return this.passengerRepository.findById(e)}async getPassengerByPhone(e){let t=l(e);return this.passengerRepository.findByPhone(t)}async getPassengers(e={}){return this.passengerRepository.findAll(e)}async getPassengersWithBookings(e={}){let{passengers:t,total:i}=await this.passengerRepository.findAll(e);return{passengers:await Promise.all(t.map(async r=>{let s=await this.bookingRepository.findByPassengerId(r.id);return{...r,bookings:s.map(a=>({id:a.id,bookingDate:a.bookingDate,status:a.status,vehicle:{name:a.vehicle.name,type:a.vehicle.type}}))}})),total:i}}async updatePassenger(e,t){if(!await this.passengerRepository.findById(e))throw Error("Passenger not found");if(t.email&&!/\S+@\S+\.\S+/.test(t.email))throw Error("Invalid email format");if(!await this.passengerRepository.updatePassenger(e,t))throw Error("Failed to update passenger");return this.passengerRepository.findById(e)}async verifyPassenger(e){let t=l(e);if(!await this.passengerRepository.findByPhone(t))throw Error("Passenger not found");return this.passengerRepository.updateVerificationStatus(t,!0)}async unverifyPassenger(e){let t=l(e);if(!await this.passengerRepository.findByPhone(t))throw Error("Passenger not found");return this.passengerRepository.updateVerificationStatus(t,!1)}async deletePassenger(e){if(!await this.passengerRepository.findById(e))throw Error("Passenger not found");return this.passengerRepository.deletePassenger(e)}async passengerExists(e){let t=l(e);return this.passengerRepository.existsByPhone(t)}async getPassengerStats(){return this.passengerRepository.getStats()}async findOrCreatePassenger(e){let t=l(e.phone),i=await this.passengerRepository.findByPhone(t);return i?((e.name||e.email)&&(await this.passengerRepository.updatePassenger(i.id,{name:e.name||i.name,email:e.email||i.email}),i=await this.passengerRepository.findById(i.id)),i):this.createPassenger(e)}async searchPassengers(e,t={}){let{passengers:i,total:r}=await this.getPassengers({page:1,limit:1e3}),s=i.filter(o=>o.name?.toLowerCase().includes(e.toLowerCase())||o.phone.includes(e)||o.email?.toLowerCase().includes(e.toLowerCase())),{page:a=1,limit:c=10}=t,d=(a-1)*c;return{passengers:s.slice(d,d+c),total:s.length}}}var w=f(535);class k{constructor(e){this.adminRepository=e}async authenticate(e,t){let i=await this.adminRepository.findByUsername(e);if(!i)return null;let r=await this.getAdminWithPassword(e);if(!r||!await(0,w.b9)(t,r.password))return null;await this.adminRepository.updateLastLogin(i.id);let s=await(0,w.FA)({adminId:i.id,username:i.username,role:i.role},process.env.JWT_SECRET||"fallback-secret");return{admin:i,token:s}}async getAdminWithPassword(e){return this.adminRepository.findByUsernameWithPassword(e)}async getAdminById(e){return this.adminRepository.findById(e)}}var u=f(3993);class S{async getDashboardStats(){let[e,t,i,r]=await Promise.all([this.database.query("SELECT COUNT(*) as count FROM bookings",[]),this.database.query("SELECT COUNT(*) as count FROM bookings WHERE status = $1",["pending"]),this.database.query("SELECT COUNT(*) as count FROM vehicles",[]),this.database.query("SELECT COUNT(*) as count FROM passengers",[])]);if(!e.success||!t.success||!i.success||!r.success)throw Error("Failed to fetch dashboard statistics");return{totalBookings:e.data?.[0]?.count||0,pendingBookings:t.data?.[0]?.count||0,totalVehicles:i.data?.[0]?.count||0,totalPassengers:r.data?.[0]?.count||0}}constructor(){this.database=u._.getDefaultProvider()}}class m{constructor(e){this.db=e}async executeQuery(e,t=[]){return this.db.query(e,t)}async select(e,t=[]){let i=await this.executeQuery(e,t);if(!i.success)throw Error(i.error||"Query failed");return i.data||[]}async insert(e,t=[]){let i=await this.executeQuery(e,t);if(!i.success)throw Error(i.error||"Insert failed");return i.meta?.last_row_id||0}async update(e,t=[]){let i=await this.executeQuery(e,t);if(!i.success)throw Error(i.error||"Update failed");return i.meta?.changes||0}async delete(e,t=[]){let i=await this.executeQuery(e,t);if(!i.success)throw Error(i.error||"Delete failed");return i.meta?.changes||0}}class A extends m{async getAvailableVehicles(e){let t=`
    ...
    `,r=`photo_${Date.now()}_${Math.random().toString(36).substr(2,9)}`;return await this.insert(i,[r,e,t.url,t.alt,+!!t.isPrimary,t.order]),{id:r,vehicleId:e,url:t.url,alt:t.alt,isPrimary:t.isPrimary,order:t.order,createdAt:new Date}}async setPrimaryPhoto(e,t){return await this.update("UPDATE vehicle_photos SET isPrimary = 0 WHERE vehicleId = $1",[e]),await this.update("UPDATE vehicle_photos SET isPrimary = 1 WHERE id = $1 AND vehicleId = $2",[t,e])>0}async deleteVehiclePhoto(e){return await this.delete("DELETE FROM vehicle_photos WHERE id = $1",[e])>0}async updateVehicle(e,t){let i=`
      ...
      WHERE id = $8
    `,r=JSON.stringify(t.features);return await this.update(i,[t.name,t.type,t.capacity,t.pricePerDay,t.description,r,+!!t.isAvailable,e])>0}async deleteVehicle(e){return await this.delete("DELETE FROM vehicles WHERE id = $1",[e])>0}}class R extends m{async create(e){let t=`
    ...
    `,i=crypto.randomUUID(),r=new Date;return await this.insert(t,[i,e.passengerId,e.vehicleId,e.bookingDate.toISOString(),e.pickupTime,e.tripType,e.pickupLocation,e.dropoffLocation||null,e.status,e.notes||null,r.toISOString(),r.toISOString()]),this.findById(i)}async findById(e){let t=`
      ...
      WHERE b.id = $1
    `,i=await this.select(t,[e]);return i.length===0?null:this.mapBookingResult(i[0])}async findAll(e={}){let{status:t,page:i=1,limit:r=10}=e,s="",a=[];t&&(s="WHERE b.status = $1",a.push(t));let c=`
      SELECT 
        b.*,
        p.id as passenger_id, p.phone as passenger_phone, p.name as passenger_name, 
        p.email as passenger_email, p.isVerified as passenger_isVerified,
    ...
    `,d=`
      ...
      FROM bookings b
      ${s}
    `,[o,y]=await Promise.all([this.select(c,[...a,r,(i-1)*r]),this.select(d,a)]),h=y[0]?.total||0;return{bookings:o.map(g=>this.mapBookingResult(g)),total:h}}async findByPassengerId(e){let t=`
      SELECT 
        b.*,
        p.id as passenger_id, p.phone as passenger_phone, p.name as passenger_name, 
        p.email as passenger_email, p.isVerified as passenger_isVerified,
    ...
    `,i=await this.select(t,[e]);return i.length===0?null:i[0]}async updateLastLogin(e){let t=`
      ...
      SET updatedAt = $1
      WHERE id = $2
    `;return await this.update(t,[new Date().toISOString(),e])>0}mapAdminResult(e){return{id:e.id,username:e.username,email:e.email,phone:e.phone,role:e.role,createdAt:new Date(e.createdAt),updatedAt:new Date(e.updatedAt)}}}class ${static{this.vehicleService=null}static{this.bookingService=null}static{this.passengerService=null}static{this.adminService=null}static{this.adminStatsService=null}static getVehicleService(){if(!this.vehicleService){let e=new A(u._.getDefaultProvider());this.vehicleService=new v(e)}return this.vehicleService}static getBookingService(){if(!this.bookingService){let e=u._.getDefaultProvider(),t=new R(e),i=new b(e),r=new A(e);this.bookingService=new T(t,i,r)}return this.bookingService}static getPassengerService(){if(!this.passengerService){let e=u._.getDefaultProvider(),t=new b(e),i=new R(e);this.passengerService=new _(t,i)}return this.passengerService}static getAdminService(){if(!this.adminService){let e=new D(u._.getDefaultProvider());this.adminService=new k(e)}return this.adminService}static getAdminStatsService(){return this.adminStatsService||(this.adminStatsService=new S),this.adminStatsService}static reset(){this.vehicleService=null,this.bookingService=null,this.passengerService=null,this.adminService=null,this.adminStatsService=null}}},V.__chunk_535=(B,P,f)=>{"use strict";function v(t){let i=new Uint8Array(t.length);for(let r=0;r<t.length;r++)i[r]=t.charCodeAt(r);return i}f.d(P,{b9:()=>n,FA:()=>b,_H:()=>e});function l(t){return btoa(String.fromCharCode(...new TextEncoder().encode(t))).replace(/=/g,"").replace(/\+/g,"-").replace(/\//g,"_")}function E(t){return v(atob(t.replace(/-+(BEGIN|END).*/g,"").replace(/\s/g,"")))}async function T(t,i,r){return await crypto.subtle.importKey("raw",v(t),i,!0,r)}async function _(t,i,r){return await crypto.subtle.importKey("jwk",t,i,!0,r)}async function w(t,i,r){return await crypto.subtle.importKey("spki",E(t),i,!0,r)}async function k(t,i,r){return await crypto.subtle.importKey("pkcs8",E(t),i,!0,r)}async function u(t,i,r){if(typeof t=="object")return _(t,i,r);if(typeof t!="string")throw Error("Unsupported key type!");return t.includes("PUBLIC")?w(t,i,r):t.includes("PRIVATE")?k(t,i,r):T(t,i,r)}function S(t){let i=Array.from(atob(t),r=>r.charCodeAt(0));return JSON.parse(new TextDecoder("utf-8").decode(new Uint8Array(i)))}if(typeof crypto>"u"||!crypto.subtle)throw Error("SubtleCrypto not supported!");var m={none:{name:"none"},ES256:{name:"ECDSA",namedCurve:"P-256",hash:{name:"SHA-256"}},ES384:{name:"ECDSA",namedCurve:"P-384",hash:{name:"SHA-384"}},ES512:{name:"ECDSA",namedCurve:"P-521",hash:{name:"SHA-512"}},HS256:{name:"HMAC",hash:{name:"SHA-256"}},HS384:{name:"HMAC",hash:{name:"SHA-384"}},HS512:{name:"HMAC",hash:{name:"SHA-512"}},RS256:{name:"RSASSA-PKCS1-v1_5",hash:{name:"SHA-256"}},RS384:{name:"RSASSA-PKCS1-v1_5",hash:{name:"SHA-384"}},RS512:{name:"RSASSA-PKCS1-v1_5",hash:{name:"SHA-512"}}};async function A(t,i,r="HS256"){if(typeof r=="string"&&(r={algorithm:r}),r={algorithm:"HS256",header:{typ:"JWT",...r.header??{}},...r},!t||typeof t!="object")throw Error("payload must be an object");if(r.algorithm!=="none"&&(!i||typeof i!="string"&&typeof i!="object"))throw Error("secret must be a string, a JWK object or a CryptoKey object");if(typeof r.algorithm!="string")throw Error("options.algorithm must be a string");let s=m[r.algorithm];if(!s)throw Error("algorithm not found");t.iat||(t.iat=Math.floor(Date.now()/1e3));let a=`${l(JSON.stringify({...r.header,alg:r.algorithm}))}.${l(JSON.stringify(t))}`;if(r.algorithm==="none")return a;let c=i instanceof CryptoKey?i:await u(i,s,["sign"]),d=await crypto.subtle.sign(s,c,v(a));return`${a}.${btoa(function(o){let y="";for(let h=0;h<o.byteLength;h++)y+=String.fromCharCode(o[h]);return y}(new Uint8Array(d))).replace(/=/g,"").replace(/\+/g,"-").replace(/\//g,"_")}`}async function R(t,i,r="HS256"){var s,a,c;if(typeof r=="string"&&(r={algorithm:r}),r={algorithm:"HS256",clockTolerance:0,throwError:!1,...r},typeof t!="string")throw Error("token must be a string");if(r.algorithm!=="none"&&typeof i!="string"&&typeof i!="object")throw Error("secret must be a string, a JWK object or a CryptoKey object");if(typeof r.algorithm!="string")throw Error("options.algorithm must be a string");let d=t.split(".",3);if(d.length<2)throw Error("token must consist of 2 or more parts");let[o,y,h]=d,g=m[r.algorithm];if(!g)throw Error("algorithm not found");let p={header:S((s=t).split(".")[0].replace(/-/g,"+").replace(/_/g,"/")),payload:S(s.split(".")[1].replace(/-/g,"+").replace(/_/g,"/"))};try{if(p.header?.alg!==r.algorithm)throw Error("INVALID_SIGNATURE");if(p.payload){let I=Math.floor(Date.now()/1e3);if(p.payload.nbf&&p.payload.nbf>I&&p.payload.nbf-I>(r.clockTolerance??0))throw Error("NOT_YET_VALID");if(p.payload.exp&&p.payload.exp<=I&&I-p.payload.exp>(r.clockTolerance??0))throw Error("EXPIRED")}if(g.name==="none")return p;let O=i instanceof CryptoKey?i:await u(i,g,["verify"]);if(!await crypto.subtle.verify(g,O,(c=h,v(atob(c.replace(/-/g,"+").replace(/_/g,"/").replace(/\s/g,"")))),(a=`${o}.${y}`,v(a))))throw Error("INVALID_SIGNATURE");return p}catch(O){if(r.throwError)throw O;return}}async function b(t,i){return await A(t,i)}async function D(t,i){try{return await R(t,i)}catch{throw Error("Invalid token")}}async function $(t){let i=new TextEncoder().encode(t);return Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256",i))).map(r=>r.toString(16).padStart(2,"0")).join("")}async function n(t,i){return await $(t)===i}async function e(t){let i=t.headers.get("authorization");if(!i||!i.startsWith("Bearer "))throw Error("No token provided");let r=i.replace("Bearer ",""),s=await D(r,process.env.JWT_SECRET||"fallback-secret"),a=s.payload||s;if(!a.adminId||!a.username||!a.role)throw Error("Invalid token payload");return a}},V);export{H as __getNamedExports};
